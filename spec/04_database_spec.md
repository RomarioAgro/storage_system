...
CHECK(quantity > 0)
UNIQUE(cell_id) WHERE quantity > 0
```
...
- хранить только один товар с положительным остатком в одной ячейке.

Если остаток товара в ячейке становится равен `0`, строка `stock_items`
удаляется. Такая ячейка считается пустой.
Положительный остаток другого товара в той же ячейке запрещен на уровне БД
partial unique index и на уровне сервисной логики.
