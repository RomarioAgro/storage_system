// MfClassic.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
UINT64 g_AuthKey = 0xffffffffffff;		// Ключ аутентификации Mifare
...
void SaveRdKey(int nKeyIdx, bool fKeyB, const MfClassicKey &nKey)
{
	...
	_tcscpy_s(p, _countof(szCfgPath) - (p - szCfgPath), _T(".ini"));
	TCHAR szKeyIdx[8];
	_itot_s(nKeyIdx, szKeyIdx, 10);
	TCHAR szAuthKey[16];
	_ui64tot_s(nKey, szAuthKey, _countof(szAuthKey), 16);
...
{
	TCHAR szAuthKey[16];
	_ui64tot_s(g_AuthKey, szAuthKey, _countof(szAuthKey), 16);
	TCHAR szCfgPath[MAX_PATH];
	GetModuleFileName(NULL, szCfgPath, _countof(szCfgPath));
	LPTSTR p = _tcsrchr(szCfgPath, _T('.'));
	_tcscpy_s(p, _countof(szCfgPath) - (p - szCfgPath), _T(".ini"));
...
{
	TCHAR szRdKeys[16];
	_ultot_s(g_nRdKeys, szRdKeys, _countof(szRdKeys), 10);
	TCHAR szCfgPath[MAX_PATH];
	GetModuleFileName(NULL, szCfgPath, _countof(szCfgPath));
	LPTSTR p = _tcsrchr(szCfgPath, _T('.'));
	_tcscpy_s(p, _countof(szCfgPath) - (p - szCfgPath), _T(".ini"));
...
void DoReadClassic(IILReader *pReader)
{
	try
	{
		...
		int nRead = 0;
		ILRCheck(pReader->ReadMfClassic(0, oBlocks.data(), (int)oBlocks.size(), &nRead));
...
void DoWriteClassic(IILReader *pReader)
{
	try
	{
		...
		DWORD nSpan = GetTickSpan(nStartTick, GetTickCount());
		_tprintf(_T("записан %d блок за %d мс\n"), nWritten, nSpan);
	}
	catch (std::exception &e)
	{
		printf("%s\r\n", e.what());
...
void WriteAuthKeyToReader(IILReader *pReader)
{
	try
	{
...
void DoEnterAuthKey()
{
	...
	MfClassicKey nAuthKey;
	if ((_tscanf_s(TEXT("%I64X"), &nAuthKey) != 1) || (nAuthKey > 0xffffffffffff))
	{
		_tprintf(TEXT("Неправильный ввод\n"));
		return;
	}
...
{
	_tprintf(TEXT("Введите номер ключа (0..15) или '=' + битовую маске (16-ричное):\n"));
	TCHAR szInput[256];
	...
	else
	{
		...
		int nKeyIdx;
		g_nRdKeys = 0;
		pToken = _tcstok_s(szInput, pszDelimiters, &p);
		while (pToken != NULL)
		{
			nKeyIdx = _ttoi(pToken);
			if ((nKeyIdx >= 0) && (nKeyIdx < 16))
				g_nRdKeys |= (1 << nKeyIdx);
			pToken = _tcstok_s(NULL, pszDelimiters, &p);
		}
	}
...
int main()
{
	...
	try
	{
		...
		while (true)
		{
			...
			_tprintf(TEXT("1 - Искать карту снова\n"));
