// Temic.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
void DoReadTemic(IILReader *pReader)
{
	try
	{
		...
		{
			pReader->DecodeTemicEmMarine(aBlocks, 2, &rUID);
			if (!rUID.IsEmpty())
			{
				CString s;
				CardUIDToStr(cEmMarine, rUID, &s);
...
void DoWriteEmMarine(IILReader *pReader)
{
	try
	{
		...
		_tprintf(TEXT("Введите номер Em-Marine: код производителя (16-ричное), серия (10-тичное), номер (10-тичное):"));
...
void DoWriteHID(IILReader *pReader)
{
	try
	{
		...
		_tprintf(TEXT("Введите номер HID: виеганд (10-тичное), код прозводителя (16-ричное), номер (10-тичное):"));
		int nWiegand, nFacility, nNumber;
		if ((_tscanf_s(TEXT("%x %d %d"), &nWiegand, &nFacility, &nNumber) != 3) ||
...
void DoInitTemic(IILReader *pReader)
{
	try
	{
		...
		DWORD nSpan = GetTickSpan(nStartTick, GetTickCount());
		_tprintf(TEXT("записан %d блок за %d мс\n"), nWritten, nSpan);
	}
	catch (std::exception &e)
	{
		printf("%s\r\n", e.what());
...
int main()
{
	...
	{
		_tprintf(TEXT("Ошибка зашрузки \"%s\": 0x%x\n"), ILR_DLL_NAME, oILRLoader.m_hr);
		return -1;
	}
