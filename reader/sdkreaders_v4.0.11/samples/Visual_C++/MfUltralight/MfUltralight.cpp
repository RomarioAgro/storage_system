// MfUltralight.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
void DoReadUltralight(IILReader *pReader)
{
	try
	{
		...
		const LPCTSTR kLocked[2] = { _T(""), TEXT("Заблокировано") };
		DWORD nPageData, n;
...
void DoWriteUltralight(IILReader *pReader)
{
	try
	{
		...
		DWORD nSpan = GetTickSpan(nStartTick, GetTickCount());
		_tprintf(_T("записано %d страница за %d мс)\n"), nWritten, nSpan);
	}
	catch (std::exception &e)
	{
		printf("%s\r\n", e.what());
