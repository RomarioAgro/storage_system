...
uses
  ...
  IL.Readers, uMfPlusKeysLvData, uClasses, uTypes;

type
  TfmMfPlusKeysDlg = class(TForm)
    btnCancel: TButton;
    btnOk: TButton;
    ListView1: TListView;
  ...
  public
    { Public declarations }
    m_oKeys         : TMfPlusKeyList;
    m_nKeyIdx       : Integer;
  end;

var
  fmMfPlusKeysDlg: TfmMfPlusKeysDlg;
...
uses
  System.Character,
  IL.RUtils, uUtils, uAppSettings;

{ TfmMfPlusKeysDlg }

procedure TfmMfPlusKeysDlg.btnOkClick(Sender: TObject);
var
  nIdx: Integer;
begin
  if (m_nKeyIdx <> -1) and SaveProperties and
    (m_nKeyIdx = m_oKeys.Count) and
    ((m_oLVD.m_rNewKey.m_rKey <> kDefaultMfPlusKey) or
    (not m_oLVD.m_rNewKey.m_sComment.IsEmpty)) then
  begin
    nIdx := m_oKeys.Add(m_oLVD.m_rNewKey);
    m_oLVD.m_rNewKey.m_rKey := kDefaultMfPlusKey;
  ...
  if m_fModified then
  begin
    g_AppSet.m_oMpKeys.Clear;
    g_AppSet.m_oMpKeys.AddRange(m_oKeys);
    g_AppSet.SaveMfPlusKeys;
    m_fModified := False;
  end;
end;

procedure TfmMfPlusKeysDlg.CMChildKey(var Msg: TCMChildKey);
var
  nIdx: Integer;
begin
  inherited;
...
procedure TfmMfPlusKeysDlg.DoDelete;
var
  oLI: TListItem;
begin
...
procedure TfmMfPlusKeysDlg.edtKeyKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfmMfPlusKeysDlg.FormClose(Sender: TObject; var Action: TCloseAction);
var
...
begin
  ...
  if g_AppSet.m_aWinSizes[swMfPlusKeysDlg] <> rSize then
    g_AppSet.SetWinSize(swMfPlusKeysDlg, rSize);
end;

procedure TfmMfPlusKeysDlg.FormCreate(Sender: TObject);
var
  rSize: TSize;
begin
  m_nKeyIdx := -1;
  m_oKeys := TMfPlusKeyList.Create;
  m_oKeys.AddRange(g_AppSet.m_oMpKeys);
  m_oLVD := TMfPlusKeysLvData.Create;
  m_oLVD.m_pKeys := m_oKeys;
  m_oLVD.OnUpdateAllView := LVDUpdateAllView;
  m_oLVD.m_rNewKey.m_rKey := kDefaultMfPlusKey;

  rSize := g_AppSet.m_aWinSizes[swMfPlusKeysDlg];
...
procedure TfmMfPlusKeysDlg.FormDestroy(Sender: TObject);
begin
  m_oLVD.Free;
  m_oKeys.Free;
end;

procedure TfmMfPlusKeysDlg.FormShow(Sender: TObject);
var
  nIdx: Integer;
begin
  nIdx := m_nKeyIdx;
...
procedure TfmMfPlusKeysDlg.ListView1CustomDrawSubItem(Sender: TCustomListView;
  Item: TListItem; SubItem: Integer; State: TCustomDrawState;
  var DefaultDraw: Boolean);
begin
  if SubItem = 1 then
...
procedure TfmMfPlusKeysDlg.ListView1Data(Sender: TObject; Item: TListItem);
begin
  m_oLVD.GetData(Item);
end;

procedure TfmMfPlusKeysDlg.ListView1DataHint(Sender: TObject; StartIndex,
  EndIndex: Integer);
begin
  m_oLVD.DataHint(StartIndex, EndIndex);
end;

procedure TfmMfPlusKeysDlg.ListView1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfmMfPlusKeysDlg.ListView1SelectItem(Sender: TObject; Item: TListItem;
  Selected: Boolean);
begin
  if Selected then
  begin
...
procedure TfmMfPlusKeysDlg.LVDUpdateAllView(Sender: TObject);
begin
  ListView1.Items.Count := m_oLVD.GetCount;
  ListView1.Invalidate;
end;

function TfmMfPlusKeysDlg.SaveProperties: Boolean;
var
  rKey: TMfPlusKey;
  fKeyOk: Boolean;
  pKI: PMpKeyInfo;
...
procedure TfmMfPlusKeysDlg.UpdateProperties;
var
  pKI: PMpKeyInfo;
begin
